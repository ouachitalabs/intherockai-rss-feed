...
def fetch_new_articles(url: Annotated[str, "URL"]) -> ArticleCollection:
        ...
        try:
            ...
            if hasattr(entry, "published"):
                try:
                    ...
                    logger.debug(
                        f"Failed to parse published date '{entry.published}': {e}"
                    )
    ...
    logger.info(
        f"Successfully processed {len(articles)} new articles, skipped {skipped_count} entries"
    )
    ...
    if articles:
        ...
        og_images = extract_og_images_batch(
            article_urls, timeout=3, delay_range=(0.1, 0.5)
        )
        ...
        logger.info(
            f"Found Open Graph images for {images_found}/{len(articles)} articles"
        )
